...
\author[1]{\fnm{Chaoxin} \sur{Wang}}\email{wangchaoxin299@outlook.com}
\author[1]{\fnm{Hongyu} \sur{Jiang}}\email{hongyu\_jiang@163.com}

\affil*[1]{\orgdiv{Department of Artificial Intelligence}, \orgname{Zhengzhou University of Industrial Technology}, \orgaddress{\city{Zhengzhou}, \postcode{451150}, \country{China}}}

\abstract{
Humming is a natural input for query-by-humming and music-education systems, yet note-level transcription remains difficult because amateur vocal queries contain soft boundaries, pitch slides, vibrato, short unvoiced gaps and intonation errors. This paper presents a compact and reproducible pipeline for monophonic humming transcription and key detection. The transcription module combines a pretrained CREPE pitch front end, frame-level acoustic descriptors and two RandomForest classifiers that separately estimate onset and offset probabilities. All transcription results are obtained with recording-grouped five-fold out-of-fold prediction on the 38 annotated recordings of the MTG-QBH/Molina corpus. The proposed system reaches 0.689 COnPOff, improving the strongest previously reported result on the same setting by 7.9 percentage points; a non-learning CREPE-plus-energy baseline reaches only 0.270. Ablation experiments show that energy, spectral flux, voicing-run descriptors and temporal lags all contribute to performance. An independent key-estimation module based on confidence-weighted pitch-class histograms and Krumhansl-Schmuckler templates obtains 60.5\% exact-key accuracy and 78.9\% mode accuracy against keys inferred from the ground-truth notes. We also find that snapping predicted pitches to the estimated key reduces transcription accuracy, suggesting that tonal information is useful as a descriptive signal but unsafe as a hard correction rule for amateur humming.
}
...
The experiments use the public MTG-QBH/Molina monophonic humming corpus. The original audio recordings are not redistributed by this work and should be obtained from the original data source. Derived experimental artifacts, including summary tables, predicted note files, paper figures and compressed out-of-fold probability/features, are available at \url{https://github.com/YIXINOWO/humming-melody-key-estimation}.

\bmhead{Code availability}
The Python scripts used to generate the reported results are available at \url{https://github.com/YIXINOWO/humming-melody-key-estimation}.
